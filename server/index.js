// Minimal Express server scaffolding for Xero OAuth + sync endpoints
...
// In-memory token store (replace with secure storage)
let xeroState = {
  connected: false,
...
app.get('/api/xero/connect', (_req, res) => {
  // TODO: Implement OAuth2 (Auth Code + PKCE). For now, simulate connection.
...
app.post('/api/xero/sync/payroll', async (_req, res) => {
  ...
  // TODO: map employees and send to Xero Payroll API
...
app.post('/api/xero/sync/reimbursements', async (_req, res) => {
  if (!xeroState.connected) return res.status(400).json({ error: 'Not connected to Xero' });
  // TODO: map reimbursements to Xero expenses
...
app.listen(PORT, () => {
  console.log(`[xero-backend] listening on http://localhost:${PORT}`);
});
